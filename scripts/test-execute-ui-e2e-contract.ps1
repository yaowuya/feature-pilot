...
function Test-ZeroMockRule([string]$gate, [string]$canonicalLine, [string[]]$forbiddenTerms) {
...
function Test-TopLevelStatementTerminatesBeforeRegistration([System.Management.Automation.Language.StatementAst]$statement) {
    ...
    if ($ifStatement.Clauses.Count -ne 1 -or $ifStatement.Clauses[0].Item1.Extent.Text.Trim() -cnotmatch '^\$true$') {
        return $false
    }
    foreach ($statementInIf in $ifStatement.Clauses[0].Item2.Statements) {
        if ($statementInIf -is [System.Management.Automation.Language.ExitStatementAst] -or
            $statementInIf -is [System.Management.Automation.Language.ReturnStatementAst] -or
            $statementInIf.Extent.Text.Trim() -match '^(?i)(?:exit|return)(?:\s|$)') {
            return $true
        }
    }
    return $false
...
Assert-Condition (Test-ZeroMockRule $gate $zeroMockLine $mockTerms) 'fp-execute allows or omits the closed real-E2E zero-mock prohibition'
...
Assert-Condition (-not (Test-ZeroMockRule (Get-ExactSecondLevelSection (Get-EffectiveNormativeText $commentedZeroMock) $gateHeading) $zeroMockLine $mockTerms)) 'mutation survived: comment-only zero-mock rule'
$fencedZeroMock = Replace-Required $skill $zeroMockLine ('~~~' + $lf + $zeroMockLine + $lf + '~~~') 'fenced-code zero-mock rule'
Assert-Condition (-not (Test-ZeroMockRule (Get-ExactSecondLevelSection (Get-EffectiveNormativeText $fencedZeroMock) $gateHeading) $zeroMockLine $mockTerms)) 'mutation survived: fenced-code zero-mock rule'
$mayUseMock = Replace-Required $skill 'It must not use' 'It may use' 'may-use mock rule'
Assert-Condition (-not (Test-ZeroMockRule (Get-ExactSecondLevelSection (Get-EffectiveNormativeText $mayUseMock) $gateHeading) $zeroMockLine $mockTerms)) 'mutation survived: fp-execute may use mock E2E data'
$permittedMock = Insert-AfterRequired $skill $zeroMockLine ($lf + 'Exception: mock is permitted.') 'permitted mock exception'
Assert-Condition (-not (Test-ZeroMockRule (Get-ExactSecondLevelSection (Get-EffectiveNormativeText $permittedMock) $gateHeading) $zeroMockLine $mockTerms)) 'mutation survived: fp-execute permits mock E2E data'
