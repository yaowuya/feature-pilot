...
$figmaArtifactSetLine = '- The process artifact set normally includes ' + $code + '.fp-execute/progress.md' + $code + ', the unique task-owner checkbox, and a valid two-end plan''s derived ' + $code + 'tasks/00-overview.md' + $code + '; Figma-derived UI scope or a task with required ' + $code + 'FIGCAP-*' + $code + ' / ' + $code + 'PRES-*' + $code + ' additionally requires ' + $code + '.fp-execute/reviews/<timestamp>-figma-review.md' + $code + ' as the independent Figma review artifact.'
...
$figmaReviewLine = 'For Figma-derived UI scope or any task with required ' + $code + 'FIGCAP-*' + $code + ' / ' + $code + 'PRES-*' + $code + ', before updating the unique task-owner checkbox or reporting this task complete, create ' + $code + '.fp-execute/reviews/<timestamp>-figma-review.md' + $code + ' using ' + $code + '$' + '{CLAUDE_PLUGIN_ROOT}/skills/fp-figma/figma-review-template.md' + $code + ' through a fresh independent read-only reviewer. The reviewer must inspect the approved Figma/static source, ' + $code + 'figma-capabilities.md' + $code + ', ' + $code + 'figma-preservation.md' + $code + ', Visual Cases, and actual diff. Only every required ' + $code + 'FIGCAP-*' + $code + ', core ' + $code + 'PRES-*' + $code + ', and core Visual Case ' + $code + 'PASS' + $code + ', with no unapproved behavior change, permits task completion; direct execution must not self-approve it.'
...
$canonicalGateLines = @($planLine, $sourceReadyLine, $visualReviewLine, $staticLine, $interactiveLine, $noSkipLine, $evidenceLine, $zeroMockLine, $browserCapabilityLine, $figmaReviewLine, $retryLine)
$registrationLines = @(
...
Assert-Condition (Test-ExactLine $effectiveSkill $figmaArtifactSetLine) 'fp-execute does not permit the mandatory independent Figma review artifact'
