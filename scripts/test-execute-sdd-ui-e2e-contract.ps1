...
$visualReviewerPath = Join-Path $root 'skills\fp-execute-sdd\visual-reviewer-prompt.md'
$visualReviewerExists = Test-Path $visualReviewerPath
$visualReviewer = if ($visualReviewerExists) { Read-Utf8 'skills\fp-execute-sdd\visual-reviewer-prompt.md' } else { '' }
...
$effectiveVisualReviewer = Get-EffectiveVerifierPrompt $visualReviewer
$gateHeading = 'UI/E2E Delivery Gate'
$verifierHeading = 'Real Browser Verification Rules'
$visualReviewerHeading = 'Visual Review Rules'
$pluginRoot = '$' + '{CLAUDE_PLUGIN_ROOT}'
$sharedLine = 'Read ' + $tick + $pluginRoot + '/skills/_shared/ui-e2e-contract.md' + $tick + ' once before executing UI-bearing work; it owns delivery levels, allowed lifecycle paths, real-E2E evidence, zero-mock rules, coverage status semantics, customer-selected browser capability, retry, and non-waivable blocking.'
...
$ledgerLine = '- For each UI case, record its delivery level, current lifecycle stage, manifest reference, independent visual-review path/verdict, separate ' + $tick + 'visualAttempt' + $tick + ', E2E attempt count, task-review attempt, E2E evidence root, coverage-matrix result, cleanup, and any ' + $tick + 'BLOCKED' + $tick + ' rationale in progress/review package evidence only; do not copy Visual Evidence Manifest fields or replace the unique task-owner checkbox as the sole completion authority.'
$stageLine = 'The controller records ' + $tick + 'SOURCE_READY' + $tick + ' from the source-derived condition/requirement, route, and real account/role; then ' + $tick + 'STATIC_UI_READY' + $tick + '. It may record ' + $tick + 'VISUAL_REVIEW_PASS' + $tick + ' only from the persisted verdict of the fresh visual reviewer below; the controller and implementer never issue that verdict.'
$visualDispatchLine = 'After ' + $tick + 'STATIC_UI_READY' + $tick + ' and before any E2E verifier launch, dispatch one fresh independent ' + $tick + 'visual-reviewer' + $tick + ' using ' + $tick + $pluginRoot + '/skills/fp-execute-sdd/visual-reviewer-prompt.md' + $tick + '. Persist its case-level review under ' + $tick + '.fp-execute/reviews/<task-id>-<case-id>-visual-review-<visual-attempt>.md' + $tick + '; only its ' + $tick + 'VISUAL_REVIEW_PASS' + $tick + ' verdict may advance the case.'
$visualAttemptLine = 'For each Task ID + Case ID, persist ' + $tick + 'visualAttempt' + $tick + ' separately from task-review ' + $tick + 'reviewAttempt' + $tick + ' and ' + $tick + 'reviewScopeId' + $tick + '; initialize it to 0, increment exactly once immediately before each visual-review dispatch, allow dispatched values 1..3 only, and never consume a task-review attempt.'
$visualFreshnessLine = 'Before E2E launch and again at task review, require current HEAD to equal the recorded Reviewed HEAD and require the manifest/reference/current/optional-diff SHA-256 to match; a mismatch at ' + $tick + 'visualAttempt' + $tick + ' 1 or 2 returns the case to ' + $tick + 'STATIC_UI_READY' + $tick + ' and requires the next ' + $tick + 'visualAttempt' + $tick + ', while a mismatch at ' + $tick + 'visualAttempt' + $tick + ' 3 is ' + $tick + 'BLOCKED' + $tick + ' and forbids a fourth dispatch.'
$visualReadOnlyLine = 'The visual reviewer is read-only and checks only the existing Visual Evidence Manifest ' + $tick + 'reference' + $tick + ', ' + $tick + 'current' + $tick + ', and optional ' + $tick + 'diff' + $tick + ' artifacts against the recorded real runtime route/state.'
$visualVerdictLine = 'Record exactly one verdict: ' + $tick + 'VISUAL_REVIEW_PASS' + $tick + ', ' + $tick + 'CANNOT_VERIFY' + $tick + ', or ' + $tick + 'FAIL' + $tick + '. Only ' + $tick + 'VISUAL_REVIEW_PASS' + $tick + ' may advance; ' + $tick + 'CANNOT_VERIFY' + $tick + ' is ' + $tick + 'BLOCKED' + $tick + '.'
$visualOutputExceptionLine = 'The reviewer may write only {VISUAL_REVIEW_OUTPUT_PATH}; every other file, working tree, index, HEAD, branch, cache, database, service, visual artifact, and external state is read-only.'
$visualAppendOnlyLine = 'The review output path is attempt-specific and append-only. If {VISUAL_REVIEW_OUTPUT_PATH} already exists, return ' + $tick + 'BLOCKED' + $tick + ' without overwriting it.'
$visualIdentityLine = 'Before verdict, record review start/end timestamps, the reviewed HEAD, and SHA-256 for the manifest, ' + $tick + 'reference' + $tick + ', ' + $tick + 'current' + $tick + ', and optional ' + $tick + 'diff' + $tick + '; these identities must still match at E2E launch.'
...
$browserCapabilityLine = 'Use only the recorded existing project runner, installed browser extension, or existing local ' + $tick + 'playwright-cli' + $tick + '. Do not install a tool or browser component, alter project manifest/lockfile/configuration/CI, overwrite an existing configuration, or upgrade unrelated dependencies. If no customer-approved usable capability exists, return ' + $tick + 'BLOCKED' + $tick + '.'
$browserAuthorityLine = 'Browser capability is authorized only when the controller and brief record the exact selected capability, customer approval / choice, target frontend root, allowed real E2E test paths, runner/config status, and scoped real E2E case.'
$browserEvidenceLine = 'Record the exact target root, selected browser capability, customer approval / choice, allowed paths/scope, runner/config status, commands, resolved version, and every task-approved E2E test/evidence change in E2E evidence.'
$controllerAuthorityLine = 'Before every required E2E verifier launch, the controller must place the exact ' + $tick + 'Browser capability' + $tick + ', ' + $tick + 'Customer approval / choice' + $tick + ', ' + $tick + 'Allowed real E2E test paths and scope' + $tick + ', and ' + $tick + 'Runner/config status' + $tick + ' in the matching Task ID + Case ID brief UI/E2E record. If any value is missing, unresolved, unavailable, or outside the approved task scope, record ' + $tick + 'BLOCKED' + $tick + ' and do not dispatch the verifier.'
$briefAuthorityHeader = '| Case ID | Target frontend root | Browser capability | Customer approval / choice | Allowed real E2E test paths and scope | Runner/config status |'
$briefAuthorityLine = '- The controller fills this authority record before verifier dispatch. A missing, unresolved, unavailable, or out-of-scope value is ' + $tick + 'BLOCKED' + $tick + '; the verifier must consume these values rather than infer them. The record never authorizes a project dependency, lockfile, configuration, CI, or browser-component change.'
$verifierAuthorityBlockedLine = 'If any authority input is missing, unresolved, unavailable, or outside the recorded scope, return ' + $tick + 'BLOCKED' + $tick + ' without E2E execution.'
...
Assert-Condition ((Test-ExactLine $gate $stageLine) -and (Test-ExactLine $gate $visualDispatchLine) -and (Test-ExactLine $gate $visualAttemptLine) -and (Test-ExactLine $gate $visualFreshnessLine) -and (Test-ExactLine $gate $staticLine) -and (Test-ExactLine $gate $interactiveLine)) 'SDD does not enforce a fresh visual-review verdict with an independent counter and HEAD/artifact freshness before the staged lifecycle advances'
Assert-Condition $visualReviewerExists 'SDD visual reviewer prompt is missing'
Assert-Condition (Test-ExactSecondLevelHeading $effectiveVisualReviewer $visualReviewerHeading) 'SDD visual reviewer is missing its effective rules section'
$visualReviewerRules = Get-ExactSecondLevelSection $effectiveVisualReviewer $visualReviewerHeading
Assert-Condition ((Test-ExactLine $visualReviewerRules $visualReadOnlyLine) -and (Test-ExactLine $visualReviewerRules $visualVerdictLine) -and (Test-ExactLine $visualReviewerRules $visualAppendOnlyLine) -and (Test-ExactLine $visualReviewerRules $visualIdentityLine) -and (Test-ExactLine $effectiveVisualReviewer $visualOutputExceptionLine)) 'SDD visual reviewer lacks its closed output-only write boundary, append-only identity evidence, read-only scope, or verdict contract'
Assert-Condition ((Test-ExactLine $effectiveVisualReviewer 'Task ID: {TASK_ID}') -and (Test-ExactLine $effectiveVisualReviewer 'Case ID: {CASE_ID}') -and (Test-ExactLine $effectiveVisualReviewer 'Visual attempt: {VISUAL_ATTEMPT} of 3') -and (Test-ExactLine $effectiveVisualReviewer 'Reviewed HEAD: {REVIEWED_HEAD}') -and (Test-ExactLine $effectiveVisualReviewer 'Visual Evidence Manifest: {VISUAL_MANIFEST_PATH}') -and (Test-ExactLine $effectiveVisualReviewer 'Write review to: {VISUAL_REVIEW_OUTPUT_PATH}')) 'SDD visual reviewer lacks exact case/visual-attempt/head-scoped inputs/output'
Assert-Condition ($visualReviewer.Contains('- Visual attempt: <1 | 2 | 3>') -and $visualReviewer.Contains('- Reviewed HEAD: <sha>') -and $visualReviewer.Contains('- Review start: <ISO timestamp>') -and $visualReviewer.Contains('- Review end: <ISO timestamp>') -and $visualReviewer.Contains('- Artifact SHA-256: <manifest; reference; current; optional diff or N/A>')) 'SDD visual reviewer output omits attempt, ordering, HEAD, or artifact identity evidence'
...
Assert-Condition ((Test-ExactLine $verifierRules $zeroMockLine) -and (Test-ExactLine $verifierRules $browserCapabilityLine) -and (Test-ExactLine $verifierRules $evidenceLine) -and (Test-ExactLine $verifierRules $blockedSourceLine)) 'SDD verifier lacks required zero-mock, browser-capability, evidence, or source-derived blocking rules'
...
Assert-Condition ((Test-ExactLine $verifierRules $browserAuthorityLine) -and (Test-ExactLine $verifierRules $browserCapabilityLine) -and (Test-ExactLine $verifierRules $browserEvidenceLine)) 'SDD verifier lacks controller-scoped browser capability authority and necessary real E2E evidence rules'
Assert-Condition ((Test-ExactLine $effectiveVerifier 'Target frontend root: {TARGET_FRONTEND_ROOT}') -and (Test-ExactLine $effectiveVerifier 'Browser capability: {BROWSER_CAPABILITY}') -and (Test-ExactLine $effectiveVerifier 'Customer approval / choice: {CUSTOMER_APPROVAL_OR_CHOICE}') -and (Test-ExactLine $effectiveVerifier 'Allowed real E2E test paths and scope: {ALLOWED_E2E_PATHS_AND_SCOPE}') -and (Test-ExactLine $effectiveVerifier 'Runner/config status: {RUNNER_CONFIG_STATUS}') -and (Test-ExactLine $effectiveVerifier $verifierAuthorityBlockedLine) -and (Test-ExactLine $normalizedBrief $briefAuthorityHeader) -and (Test-ExactLine $normalizedBrief $briefAuthorityLine)) 'SDD controller, brief, and verifier do not share exact browser capability authority fields'
foreach ($surface in @(
    @{ Name = 'task brief'; Text = $brief }
    @{ Name = 'review package'; Text = $package }
)) {
    Assert-Condition ($surface.Text.Contains('`.fp-execute/e2e/<task-id>/<case-id>/` or `N/A`') -and $surface.Text.Contains('`.fp-execute/e2e/<task-id>/<case-id>/coverage-matrix.md` or `N/A`')) "$($surface.Name) makes E2E paths mandatory for static-only cases"
}

Assert-Condition ($brief.Contains('## UI/E2E Delivery Contract (frontend/UI only)') -and $brief.Contains('Visual Evidence Manifest reference') -and $brief.Contains('Visual reviewer handoff') -and $brief.Contains('visual-review-<visual-attempt>.md') -and $implementer.Contains('must never self-confirm ' + $tick + 'FRONTEND_E2E_PASS' + $tick) -and $reviewer.Contains('both the existing visual evidence and the independent E2E verifier evidence') -and $reviewer.Contains('persisted fresh visual-review artifact') -and $reviewer.Contains('review end precedes E2E start') -and $reviewer.Contains('current HEAD equals the recorded Reviewed HEAD') -and $package.Contains('## UI/E2E Delivery Evidence (frontend/UI only)') -and $package.Contains('Independent visual review') -and $package.Contains('visual-review-<visual-attempt>.md') -and $fix.Contains('must not be converted into review debt, ' + $tick + 'PASS_WITH_NOTES' + $tick + ', or a manual waiver')) 'SDD templates do not carry append-only visual-review identity/order evidence and independent staged UI/E2E gates'
...
$globalInstall = Insert-AfterRequired $verifier $browserCapabilityLine ($lf + 'Exception: global install is permitted.') 'global-install exception'
