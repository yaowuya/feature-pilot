...
- update/install 命令成功不是完成证据。源码、Codex 源、Codex cache、Claude cache 的核心文件哈希必须一致；`~/.dsh/skills`（或 `$DSH_HOME/skills`）中的 `fp-*` 与 `_shared/` 必须与仓库 `skills/` 逐文件哈希一致。
- DSH 目标从 `$DSH_HOME` 或 `~/.dsh` 解析，只替换该根下本仓库拥有的顶层条目（`fp-*` 与 `_shared`），不得清理或改写其他用户技能。
...
| DSH 技能根 `~/.dsh/skills` 不存在或缺失 `fp-*` | 正常模式会创建并同步；`-VerifyOnly` 报告未安装/差异，不自动写入 |
| DSH 技能根出现非本仓库顶层条目 | 只校验并替换 `fp-*` 与 `_shared`，报告其他条目但不删除 |
| CLI 提示需要重启 | 完成校验后明确提示 restart/new task；DSH 无需重启 |
