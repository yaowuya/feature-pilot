...
- Artifact layout is governed by the mandatory shared contract in `artifact-layout.md`. Every PRD, proposal, design, and task-plan Producer or Consumer must read and follow it for canonical paths, semantic splitting, safety limits, manifests, ownership, overview rules, resolution, and historical structural-conflict rejection. There is no compatibility fallback.
