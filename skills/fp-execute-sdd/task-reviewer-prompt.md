...
10. For every UI case, inspect the persisted fresh visual-review artifact and verify its `visualAttempt`-specific path/counter is independent from task-review `reviewAttempt`, while its reviewed HEAD and manifest/reference/current/optional-diff SHA-256 still match the reviewed evidence. Verify current HEAD equals the recorded Reviewed HEAD and every recorded artifact hash still matches; a mismatch at `visualAttempt` 1 or 2 requires the next visual attempt from `STATIC_UI_READY`, while a mismatch at attempt 3 is `BLOCKED` and forbids a fourth dispatch. Verify the review end precedes E2E start and that it issued `VISUAL_REVIEW_PASS` before any E2E launch. For every required UI/E2E case, then inspect the independent `.fp-execute/e2e/<task-id>/<case-id>/e2e-result.md` and `coverage-matrix.md`. Verify command/environment/destination/timestamps/attempts/test IDs/artifacts/cleanup, real browser UI provenance, source-derived coverage, and `Mocked Core API: false` for business-flow. Reject a controller/implementer-issued visual pass, mocks, direct API/backend setup bypassing UI, a screenshot substituted for E2E, skipped/waived required E2E, or missing safe-real-environment evidence. Verify both the existing visual evidence and the independent E2E verifier evidence. Keep browser interaction evidence separate from screenshot evidence and verify observable flows exercise the approved states.
