...
Combined task review verdict is PASS only when `Spec Compliance: PASS`, `Code Quality: APPROVED`, and no Critical/Important finding remains. Minor findings may be recorded without making the combined verdict fail. Any other combination is non-pass and must follow a defined transition. `NEEDS FIXES` with only Minor findings is an invalid reviewer combination: at attempt 1 or 2, append the inconsistency and exact Minor findings to the ledger, increment the same scope, and dispatch a corrected fresh reviewer without a fixer; at attempt 3, classify the Minor findings as non-blocking review debt and stop the scope without a fourth review.
...
4. Create or refresh `.fp-execute/packages/final-review-package.md`, then start an independent final review scope at attempt 1. fp-review is required for final review scope; use it with the package and resolved artifacts as inputs. If it is unavailable, record `BLOCKED` and repair/reinstall the FeaturePilot plugin instead of substituting the task-schema reviewer. Persist every final review result and path in the ledger.
