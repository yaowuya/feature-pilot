...
- **2.2 `cw-cornerstone`**：默认版本基线为 `cw-cornerstone==0.6.11`。检查项目声明版本、安装来源及现有约束，并在生成方案前从项目实际配置的软件源查询可用的最新版本，记录查询来源、查询时间、最新版本和当前版本。仅目录或声明存在不能证明运行时可用；未完成查询时不得把 `0.6.11` 描述为最新版。
...
项目已有固定版本时优先沿用，不主动降级或改变依赖主版本。若无法访问项目软件源或无法确认同名包，应将最新版标记为 `[无法判断]`，说明受阻原因和人工查询方式，不得猜测版本。查到新版本只纳入详细方案，说明兼容性、升级风险和验证方式；用户确认前不得修改依赖。缺少运行或目标环境证据时标记 `[待验证]`。
