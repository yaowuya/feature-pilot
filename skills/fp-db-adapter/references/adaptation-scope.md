...
默认方案不逐项展开全部内部记录：将已适配和不适用项按分组合并到“已适配与跳过”，只把需要适配、待验证和无法判断项转换为 `A`、`V`、`Q` 适配列表并展开明细。用户要求完整检查明细，或结论存在争议、证据冲突、高风险、无法判断时，再输出逐项审计表。压缩展示不得减少实际检查项或隐藏待处理项。
...
- **2.2 `cw-cornerstone`**：默认版本基线为 `cw-cornerstone==0.6.11`，默认查询源为 `https://bkrepo.cwoa.net/pypi/aiops/kingeye-pypi/simple`。检查项目声明版本、安装来源及现有约束，并在生成方案前从该源查询可用的最新版本；若项目明确配置了其他实际安装源，则以项目源为准并说明差异。记录查询来源、查询时间、最新版本和当前版本。仅目录或声明存在不能证明运行时可用；未完成查询时不得把 `0.6.11` 描述为最新版。
...
项目已有固定版本时优先沿用，不主动降级或改变依赖主版本。查询默认 BKRepo 源时优先复用项目或当前环境已有的 pip 认证配置，不在命令、输出或方案中展示用户名、密码、Token 或含凭据 URL，也不要求用户把凭据直接发到对话中。若因 `401`、网络限制或包名无法确认而查询失败，将最新版标记为 `[无法判断]`，记录不含敏感信息的失败原因和人工查询方式，不得猜测版本。查到新版本只纳入详细方案，说明兼容性、升级风险和验证方式；用户确认前不得修改依赖。缺少运行或目标环境证据时标记 `[待验证]`。
