...
每项都使用 `SKILL.md` 定义的固定状态，并在内部保留编号、证据、判断依据和处置结论。某分组不适用时也保留分组，逐项标记 `[不适用，跳过]`。适配范围为 `all` 时，达梦与 OceanBase 必须分别取证和判定。

默认方案先按 `SKILL.md` 的固定格式列出全部十个分组及简要说明，再展示适配结论。不要在分组概览中展开检查结果。后续将已适配和不适用项按分组合并到“已适配与跳过”，只把需要适配、待验证和无法判断项转换为 `A`、`V`、`Q` 适配列表并展开明细。用户要求完整检查明细，或结论存在争议、证据冲突、高风险、无法判断时，再输出逐项审计表。压缩展示不得减少实际检查项或隐藏待处理项。
...
- **2.2 `cw-cornerstone`**：默认版本为 `cw-cornerstone==0.7.2`。检查项目声明版本、安装来源及现有约束，但不主动访问 pip 源或其他仓库查询新版本。方案中将版本选择列为确认项，提示用户确认使用默认 `0.7.2`，或明确指定其他版本；不得把 `0.7.2` 描述为最新版。
...
项目已有固定版本时记录当前版本及与默认 `0.7.2` 的差异，不主动降级、升级或改变依赖主版本。版本确认项不触发联网查询，也不因未查询最新版标记 `[无法判断]`；用户确认使用 `0.7.2` 或指定其他版本后，再把对应的兼容性、变更风险和验证方式纳入实施。用户确认前不得修改依赖。缺少运行或目标环境证据时标记 `[待验证]`。
...
- **6.2 达梦字段与标识符**：检查超长 `CharField`、`AutoField`/`BigAutoField`、`desc`/`key` 等关键字和带反引号 SQL。对于数据库中尚不存在的字段，若 `CreateModel` 或 `AddField` 原计划使用 `CharField(max_length > 2000)`，则直接新增为 `TextField`；`max_length == 2000` 仍使用 `CharField`。对于数据库中已经存在的字段，从 `CharField` 转为 `TextField` 时禁止直接使用 `AlterField`，必须在达梦替换 migration 中使用 `cw_cornerstone.django.migrations.operations.fields.CloneField`。
