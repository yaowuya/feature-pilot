...
`cw-cornerstone==0.6.11` 是默认基线，不代表当前最新版。每次适配都先从目标项目实际配置的软件源查询 `cw-cornerstone` 的最新可用版本，并在方案中记录软件源、查询时间、当前版本、最新版本和版本差异。早期适配文档提到 `0.6.9`，但不得回退；查到高于 `0.6.11` 的版本时，先评估 Python、Django、达梦、OceanBase 和迁移补丁兼容性，再把是否升级及验证方式写入方案。无法访问软件源时标记 `[无法判断]`，不得把默认基线写成最新版或猜测版本。
