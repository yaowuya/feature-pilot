...
设计产物遵循下方 **Canonical design layout**；任务计划遵循下方 **Canonical task layout**。

### Canonical design layout

新设计只写入 `fp-docs/changes/<slug>/design/`：`design/00-index.md` 为总入口；实际涉及后端/前端时分别使用稳定入口 `design/backend.md`、`design/frontend.md`。任一端预计超过 500 行时，稳定入口只保留摘要与导航，详细内容写入对应的 `design/backend/00-index.md` + 编号子系统文件，或 `design/frontend/00-index.md` + 编号区域文件。

### Canonical task layout

Small task plans keep executable tasks in stable files `tasks/plan-backend.md` and `tasks/plan-frontend.md`. If an end-specific plan exceeds 500 lines, keep the stable file as concise constraints/navigation/coverage and write executable tasks under `tasks/backend/00-index.md` plus deterministic numbered fragments, or `tasks/frontend/00-index.md` plus deterministic numbered fragments. Keep fragments <=200 lines where practical. Each executable task has one stable end-prefixed ID and one real checkbox in its owner file; numbering continues across fragments. Create checkbox-free `tasks/00-overview.md` when both ends are planned or either end is split, so cross-end order, dependencies, coverage, and progress roll-up have one index. A single small end stays simple.

### Legacy read compatibility

对已有活跃变更，只有 canonical entrypoint 不存在时才可回退读取根目录旧 `design-backend.md` / `design-frontend.md`。所有新写入和修订必须使用 canonical layout；不得创建或更新旧路径。
...
- 按实际涉及端生成设计文档：涉及后端才生成 `design/backend.md`；涉及前端/UI 才生成 `design/frontend.md`；始终生成 `design/00-index.md`；没有前端计划时不得生成前端设计入口、分片或空占位文件。
...
- 用工具确认 `design/00-index.md` 及实际涉及端的稳定入口存在；分片设计还必须确认对应端 `00-index.md` 列出的每个编号文件存在。
...
- 没有前端计划，或 canonical/legacy 解析都不存在已确认的前端设计入口时，不得生成 `plan-frontend.md` 或前端任务占位文件。
- 后端/前端任务都按项目实际分层、已确认设计和现有代码依赖顺序生成；只覆盖真实涉及的模型、服务、接口、权限、客户端、状态、路由、页面/组件、样式和验证边界，不固定框架术语或工具名。
- 两端都有计划或任一端拆分时生成 `tasks/00-overview.md`，只记录入口、唯一 owner path、跨端顺序、依赖、覆盖和进度汇总，不复制 task checkbox。

阶段完成检查：
- 用工具确认任务计划文件存在；若存在 `tasks/backend/00-index.md` 或 `tasks/frontend/00-index.md`，还必须确认 `tasks/00-overview.md` 和 index 列出的所有 fragment 存在、没有 unindexed fragment，不得依赖入口链接或 glob 顺序。
- 确认 task IDs/checkbox 在 owner files 中各出现一次，overview/index/拆分稳定入口不含 task checkbox，跨端顺序覆盖全部 task IDs 且依赖无环，overview progress summary 等于 owner checkbox 派生计数。
...
- 执行前必须做 Pre-flight Plan Review；发现计划冲突先汇总处理，不要边执行边猜；执行者先解析唯一 task-owner files，若存在端内 index 则按 index 顺序读取 fragments，并按 `tasks/00-overview.md`（如存在）合并跨端顺序。
