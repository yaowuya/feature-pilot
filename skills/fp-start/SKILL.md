...
- 执行验证命令并记录结果。
- review attempt 1 或 2 未通过时，由所选 executor 按有限状态机记录问题、定向修复并进入下一次 review；只有第 3 次后仍存在主流程阻断、发现计划缺陷或需要用户决策时才暂停。非阻断 finding 必须记录为 review debt，不得触发第 4 次 review，也不得因 reviewer 的 `Ready for next task` 单独决定暂停。
- SDD 模式第一版必须串行派发实现任务，不要让多个 implementer 并行修改同一工作区。

执行完所有任务后加载 `fp-review`，但 selected executor owns a final review scope with a maximum of three reviews，并负责 verdict/severity 映射、修复、attempt 恢复和 review debt/主流程阻断分类。final review 通过，或第 3 次后只剩已记录的非阻断 review debt 时，才提示运行 `/fp-archive`；主流程阻断仍存在时不得建议归档。
