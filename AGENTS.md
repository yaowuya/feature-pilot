...
不得使用 `irm`、`curl`、远程安装脚本或 `npx`。缺少 npm 时不得自动安装 Node.js。对应当前 CLI 状态，`fp-init` 的安装 / Agent MCP 配置 / 首次建图由一次批准覆盖并按顺序逐步汇报（MCP 会修改用户级配置，须在选项中明确）；CLI 已安装时 MCP 配置与首次建图合并为一次确认。自动安装选择本身包含当前项目首次建图授权。
